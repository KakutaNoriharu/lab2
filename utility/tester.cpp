...
public:
    unsigned int ParameterID; // 現在のパラメータID
    unsigned int ParameterProgress; // 現在の進行度
    unsigned int ParameterTotal; // 全パラメータの組み合わせ数
    unsigned int clear_size; // 表示した文字列の長さ
    ...
    // コンストラクタ
    ParameterManager(unsigned int prg) : ParameterID(0), ParameterProgress(prg), ParameterTotal(1) {
        SetName="";
    ...
    void print() {
        if (ParameterProgress <= ParameterTotal) {
            ...
            for (unsigned int i = parameter_itr.size() - 1; i < parameter_itr.size(); --i) {
